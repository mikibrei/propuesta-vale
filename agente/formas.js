const _=[{n:"Billete",d:"S\xF3lo las l\xEDneas del billete: se engruesan en la sombra, sin borde."},{n:"Grano",d:"Tramado de grano; un borde n\xEDtido y el otro hecho polvo."},{n:"N\xFAmeros",d:"La forma hecha de d\xEDgitos que cambian."},{n:"Manitos",d:"La forma llena, con dos manos de verdad."}],M=[["Reposo","un c\xEDrculo que respira"],["Escuchando","una p\xEDldora que se ensancha con tu voz"],["Pensando","una gota que gira despacio"],["Buscando","sale un pedacito, da la vuelta y vuelve"],["Hablando","un globo que late con las s\xEDlabas"],["Confirm\xE1","medio c\xEDrculo que asiente"],["Subi\xF3","un tri\xE1ngulo que sube"],["Baj\xF3","un tri\xE1ngulo que se hunde"],["No puedo","se desinfla y se aplasta"],["Leyendo tu cartera","una tarjeta que se lee"],["Vigilando","una lente que mira de lado a lado"],["Te avisa","salta, y le aparece un aviso"]],S="attribute vec2 aP; varying vec2 vP; void main(){ vP = aP; gl_Position = vec4(aP, 0.0, 1.0); }",L=`precision highp float;
...
}`;function z(){const t=document.createElement("canvas");t.width=640,t.height=64;const o=t.getContext("2d");o.fillStyle="#000",o.fillRect(0,0,640,64),o.fillStyle="#fff",o.font="600 50px ui-monospace, Menlo, monospace",o.textAlign="center",o.textBaseline="middle";for(let i=0;i<10;i++)o.fillText(String(i),i*64+32,34);return t}let f=null,e=null,u=null,g=!1;function y(){const t=(s,r)=>{const l=e.createShader(s);if(e.shaderSource(l,r),e.compileShader(l),!e.getShaderParameter(l,e.COMPILE_STATUS))throw new Error(e.getShaderInfoLog(l));return l},o=e.createProgram();if(e.attachShader(o,t(e.VERTEX_SHADER,S)),e.attachShader(o,t(e.FRAGMENT_SHADER,L)),e.linkProgram(o),!e.getProgramParameter(o,e.LINK_STATUS))throw new Error(e.getProgramInfoLog(o));e.useProgram(o);const i=e.createBuffer();e.bindBuffer(e.ARRAY_BUFFER,i),e.bufferData(e.ARRAY_BUFFER,new Float32Array([-1,-1,1,-1,-1,1,1,1]),e.STATIC_DRAW);const n=e.getAttribLocation(o,"aP");e.enableVertexAttribArray(n),e.vertexAttribPointer(n,2,e.FLOAT,!1,0,0);const c=e.createTexture();e.bindTexture(e.TEXTURE_2D,c),e.pixelStorei(e.UNPACK_FLIP_Y_WEBGL,!0),e.texImage2D(e.TEXTURE_2D,0,e.RGBA,e.RGBA,e.UNSIGNED_BYTE,z()),e.texParameteri(e.TEXTURE_2D,e.TEXTURE_MIN_FILTER,e.LINEAR),e.texParameteri(e.TEXTURE_2D,e.TEXTURE_WRAP_S,e.CLAMP_TO_EDGE),e.texParameteri(e.TEXTURE_2D,e.TEXTURE_WRAP_T,e.CLAMP_TO_EDGE);const a=s=>e.getUniformLocation(o,s);u={t:a("uT"),tex:a("uTex"),est:a("uEst"),px:a("uPx"),voz:a("uVoz"),tinta:a("uTinta"),deA:a("uDeA"),deB:a("uDeB"),deQ:a("uDeQ"),mez:a("uMez")},g=!0}let m=[];const b=1.1,q=matchMedia("(prefers-reduced-motion: reduce)"),F=performance.now();let p=q.matches,E=!1,T=null;function U(t){T=t}let R=1,w=0,v=0;function k(t){R=t}let A=1,D=1;function N(t){D=t}const B=t=>T??Math.max(0,Math.sin(t*7.3)*Math.sin(t*1.9+1)*.8+.2*Math.sin(t*13));function I(){if(!g)return;const t=performance.now();v||(v=t),A+=(D-A)*Math.min(1,(t-v)/1e3*3),w+=Math.min(.1,(t-v)/1e3)*R,v=t;const o=p?2:w,n=document.documentElement.dataset.tema==="oscuro"?[.93,.94,.95]:[.082,.09,.102],c=Math.min(2,devicePixelRatio||1);e.uniform1f(u.t,o),e.uniform1f(u.voz,p?.4:Math.min(1.4,B(o)*A)),e.uniform3f(u.tinta,...n);for(const a of m){const s=a.cv.getBoundingClientRect();if(s.bottom<-50||s.top>innerHeight+50||!s.width)continue;const r=Math.round(s.width*c);a.cv.width!==r&&(a.cv.width=r,a.cv.height=r),f.width!==r&&(f.width=r,f.height=r),e.viewport(0,0,r,r),e.clearColor(0,0,0,0),e.clear(e.COLOR_BUFFER_BIT),e.uniform1f(u.tex,a.tex),e.uniform1f(u.est,a.est),e.uniform1f(u.px,2/r),a.pend!=null&&a.t0&&(performance.now()-a.t0)/1e3>=b&&a.poner(a.pend);const l=a.t0&&!p?Math.min(1,(performance.now()-a.t0)/1e3/b):1,h=l*l*(3-2*l);e.uniform1f(u.mez,h),e.uniform1f(u.deA,a.de?a.de.a:a.est),e.uniform1f(u.deB,a.de?a.de.b:a.est),e.uniform1f(u.deQ,a.de&&a.de.q<1?a.de.q:0),e.drawArrays(e.TRIANGLE_STRIP,0,4),a.x.clearRect(0,0,r,r),a.x.drawImage(f,0,0)}}function C(){E=!1,I(),!p&&m.some(t=>{const o=t.cv.getBoundingClientRect();return t.cv.isConnected&&o.width&&o.bottom>-50&&o.top<innerHeight+50})&&d()}function d(){E||(E=!0,requestAnimationFrame(C))}q.addEventListener("change",()=>{p=q.matches,d()}),new MutationObserver(d).observe(document.documentElement,{attributes:!0,attributeFilter:["data-tema"]}),addEventListener("resize",d),addEventListener("scroll",d,{passive:!0});function j(t,{solo:o=null,desde:i=1,rotulos:n=null}={}){const c=_.map((a,s)=>[a,s]).filter(([,a])=>!o||o.includes(a));t.innerHTML=c.map(([a,s],r)=>`
<section class="sec">
  <div class="cab"><span class="n">${String(i+r).padStart(2,"0")}</span><h2>${a.n}.</h2><p>${a.d}</p></div>
  <div class="grilla g4">${M.map(([l,h],x)=>`<div class="celda"><div class="lienzo"><canvas data-tex="${s}" data-est="${x}" aria-hidden="true"></canvas></div><b>${n?n[x][0]:l}</b><span>${n?n[x][1]:h}</span></div>`).join("")}</div>
</section>`).join(""),P(),m=[...t.querySelectorAll("canvas[data-tex]")].map(a=>({cv:a,x:a.getContext("2d"),tex:+a.dataset.tex,est:+a.dataset.est})),d()}function P(){if(!e&&(f=document.createElement("canvas"),e=f.getContext("webgl",{premultipliedAlpha:!0,antialias:!1,preserveDrawingBuffer:!0}),e)){try{y()}catch(t){console.warn("El dibujo no compil\xF3:",t)}f.addEventListener("webglcontextlost",t=>{t.preventDefault(),g=!1}),f.addEventListener("webglcontextrestored",()=>{try{y(),d()}catch(t){console.warn(t)}})}}function G(t,o=2,i=0){P();const n={cv:t,x:t.getContext("2d"),tex:o,est:i,t0:0,de:{a:i,b:i,q:1},pend:null};return n.poner=c=>{const a=performance.now();if((n.t0&&!p?Math.min(1,(a-n.t0)/1e3/b):1)<1){n.pend=c;return}n.pend=null,c!==n.est&&(n.de={a:n.est,b:n.est,q:1},n.est=c,n.t0=a,d())},m=m.filter(c=>c.cv.isConnected),m.push(n),d(),{poner:n.poner,parar(){m=m.filter(c=>c!==n)}}}export{N as fijarExpresion,k as fijarRitmo,U as fijarVoz,j as montar,G as montarVivo};
